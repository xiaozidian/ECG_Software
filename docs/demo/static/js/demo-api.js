...
/* Browser-only demo API: one uploaded user-confirmed simulated excerpt plus formula cases. */
(() => {
  const RATE=200,DEFAULT_DURATION=1800,COUNT=10,STORE_KEY="cardioinsight-pages-demo-v3";
  const UPLOADED_INDEX=4,UPLOADED_ASSET_BASE="static/demo-data/uploaded-sim-af-001";
  const uploaded=window.__CARDIOINSIGHT_UPLOADED_CASE__||null;
  ...
  const diagnoses=["窦性心律","偶发室性候选","偶发室上性候选","心率变异","阵发性房颤样特征","高心率片段","长 RR 候选","基线漂移","多类事件","常规复核"];
  ...
  function indexOf(caseId){if(uploaded&&caseId===uploaded.case_id)return UPLOADED_INDEX;const m=/^SYNTH-(\d{3})$/.exec(caseId||""),i=m?Number(m[1])-1:-1;return i>=0&&i<COUNT&&!(uploaded&&i===UPLOADED_INDEX)?i:-1}
  const durationFor=index=>index===UPLOADED_INDEX&&uploaded?Number(uploaded.technical.duration_seconds_raw):DEFAULT_DURATION;
  const afAt=(index,time)=>index===UPLOADED_INDEX&&uploaded&&uploaded.simulation_profile.rhythm_candidate_windows_s.some(([start,end])=>time>=start&&time<end);

  function beats(index){
    if(beatCache.has(index))return beatCache.get(index);
    if(index===UPLOADED_INDEX&&uploaded){const list=copy(uploaded.beats);beatCache.set(index,list);return list}
    const list=[],duration=durationFor(index),base=54+index*4;let time=.62,n=0;
    while(time<duration){
      let rr=60000/base*(1+.055*Math.sin(n*.071+index)+.025*Math.sin(n*.019));
      if(n>0&&n%503===0)rr=2680+index*18;
      rr=Math.max(360,Math.min(3000,rr));
      const nextTime=time+rr/1000;if(nextTime>=duration)break;
      let group=1;if(n>0&&n%(101-index%3)===0)group=3;
      else if(n>0&&n%(73+index%4)===0)group=2;
      else if(n>0&&n%347===0)group=34;
      time=nextTime;
  ...
  function baseCase(index){
    ...
    if(index===UPLOADED_INDEX&&uploaded){const item=copy(uploaded);delete item.beats;caseCache.set(index,item);return item}
    ...
    const summary={total_beats:valid.length,avg_hr:avg,min_hr:Math.round(Math.min(...rr.map(x=>60000/x))),max_hr:Math.round(Math.max(...rr.map(x=>60000/x))),ventricular_beats:counts[3],supraventricular_beats:counts[2],longest_rr_s:round(Math.max(...rr)/1000,3),sdnn_ms:62+index,sdann_ms:48+index,sdnn_index_ms:36+index,rmssd_ms:31+index*.8,pnn50_pct:8+index*.7,triangular_index:18+index*.5};
    const conclusion=`第 ${index+1} 例纯合成浏览器演示。所有候选与统计仅展示交互，不代表真实患者或临床结论。`;
    const item={case_id:`SYNTH-${String(index+1).padStart(3,"0")}`,metadata:{name:`合成${num}号`,patient_id:`DEMO-${String(index+1).padStart(4,"0")}`,sex:index%2?"女":"男",age:28+index*5,bed:"在线合成演示",clinical_diagnosis:`${diagnoses[index]}（纯合成候选）`,requesting_physician:"虚构演示",start_time:`2026-08-${num} 09:00`,start_iso:`2026-08-${num}T09:00:00`,duration_text:"30 分钟"},summary,conclusion,technical:{sample_rate_hz:RATE,independent_channels:8,derived_leads:12,sample_format:"deterministic mathematical formula",units:"µV（合成演示，无计量含义）",duration_seconds_raw:DEFAULT_DURATION,raw_size_bytes:0,report_pages:1},integrity:{manifest_available:true,algorithm:"DETERMINISTIC-SYNTHETIC-V3",file_count:0,case_sha256:"",source_version_warning:false,raw_patient_data:false},simulation_profile:null,active:true,phi_masked:true,report_image_urls:[],generated_report_url:"#synthetic-demo"};
    caseCache.set(index,item);return item;
  }
  function annotationList(caseId){const i=indexOf(caseId),isUploaded=i===UPLOADED_INDEX&&uploaded,candidate=isUploaded?uploaded.simulation_profile.rhythm_candidate_windows_s[0][0]:120+i*7,initial=[{id:-(i+1),sample_index:candidate*RATE,lead:"II",category:isUploaded?"rhythm":"note",label:isUploaded?"源模拟记录房颤候选片段":"合成演示标记",note:isUploaded?"直接读取已上传的用户确认模拟 DATA 片段":"由浏览器演示模拟器生成",created_by:isUploaded?"uploaded-simulated-case":"demo-generator",created_at:"2026-08-12 09:00:00"}];return initial.concat(saved.annotations[caseId]||[])}
  ...
  function present(index,detailed=false){
    ...
    if(detailed){const all=beats(index),valid=all.filter(x=>x.group!==34),rr=valid.map(x=>x.rr_ms),groups={};all.forEach(x=>groups[x.group]=(groups[x.group]||0)+1);const longest=valid.reduce((a,b)=>a.rr_ms>b.rr_ms?a:b);item.calculated={record_count:all.length,valid_beats:valid.length,first_beat_time_s:valid[0]?.time_s||0,group_counts:Object.fromEntries(Object.entries(groups).map(([k,v])=>[String(k),v])),avg_hr_from_duration:round(valid.length*60/durationFor(index)),avg_hr_from_rr:round(60000/(rr.reduce((a,b)=>a+b,0)/rr.length)),longest_rr_ms:longest.rr_ms,longest_rr_time_s:longest.time_s,min_rr_ms:Math.min(...rr),format_verified:true};item.report_workflow=report(item);item.annotations=annotationList(item.case_id)}
    return item;
  }
  function trend(index){const duration=durationFor(index),base=baseCase(index).summary.avg_hr,points=[];for(let t=30;t<duration;t+=60){const count=beats(index).filter(x=>x.group!==34&&x.time_s>=t-30&&x.time_s<t+30).length;points.push({time_s:t,hr:index===UPLOADED_INDEX&&uploaded?count:round(base+9*Math.sin(t/247+index)+4*Math.sin(t/73),1)})}const values=points.map(x=>x.hr);return {bin_seconds:60,points,min_hr:Math.min(...values),max_hr:Math.max(...values)}}
  function hrv(index){const source=baseCase(index).summary,adjust=(value,delta)=>Number.isFinite(value)?round(value+delta):null;return {source,calculated:{nn_count:beats(index).filter(x=>x.group===1).length,mean_nn_ms:round(60000/source.avg_hr),sdnn_ms:adjust(source.sdnn_ms,1.4),sdann_ms:adjust(source.sdann_ms,.8),sdnn_index_ms:adjust(source.sdnn_index_ms,1.1),rmssd_ms:adjust(source.rmssd_ms,.6),pnn50_pct:adjust(source.pnn50_pct,.3),triangular_index:adjust(source.triangular_index,.2),method:index===UPLOADED_INDEX&&uploaded?"上传模拟 EBI 片段的 N-N 序列":"纯合成 RR 序列"}}}
  ...
  function formulaWaveform(index,params,forced={}){
    const total=durationFor(index),start=Math.max(0,Math.min(total-1,Number(forced.start??params.get("start")??0))),duration=Math.max(.5,Math.min(120,total-start,Number(forced.duration??params.get("duration")??10))),leads=forced.leads||String(params.get("leads")||"II,V1,V5").split(",").filter(x=>x in scales),max=Math.max(200,Math.min(5000,Number(forced.maxPoints??params.get("max_points")??4000))),count=Math.max(200,Math.min(max,Math.round(duration*RATE)+1)),data={};
  ...
  let uploadedWaveformPromise=null;
  async function uploadedWaveformBuffer(){
    if(!uploadedWaveformPromise)uploadedWaveformPromise=nativeFetch(`${UPLOADED_ASSET_BASE}/waveform.bin`).then(response=>{if(!response.ok)throw new Error(`模拟波形资源加载失败（${response.status}）`);return response.arrayBuffer()});
    return uploadedWaveformPromise;
  }
  function displayFilter(values){if(!values.length)return values;const highpass=Math.exp(-2*Math.PI*.5/RATE),lowpass=1-Math.exp(-2*Math.PI*40/RATE);let baseline=values[0],low=0;return values.map(value=>{baseline=highpass*baseline+(1-highpass)*value;const high=value-baseline;low+=lowpass*(high-low);return round(low,2)})}
  function sourceLead(view,sample,lead){const offset=sample*16,read=channel=>view.getInt16(offset+channel*2,true),leadI=read(0),leadII=read(1);if(lead==="I")return leadI;if(lead==="II")return leadII;if(lead==="III")return leadII-leadI;if(lead==="aVR")return-(leadI+leadII)/2;if(lead==="aVL")return leadI-leadII/2;if(lead==="aVF")return leadII-leadI/2;return read({V1:2,V2:3,V3:4,V4:5,V5:6,V6:7}[lead]??1)}
  async function sourceWaveform(params,forced={}){
    const buffer=await uploadedWaveformBuffer(),view=new DataView(buffer),total=durationFor(UPLOADED_INDEX),start=Math.max(0,Math.min(total-1,Number(forced.start??params.get("start")??0))),duration=Math.max(.5,Math.min(120,total-start,Number(forced.duration??params.get("duration")??10))),leads=forced.leads||String(params.get("leads")||"II,V1,V5").split(",").filter(x=>x in scales),max=Math.max(200,Math.min(12000,Number(forced.maxPoints??params.get("max_points")??4000))),startSample=Math.floor(start*RATE),sampleCount=Math.min(Math.floor(duration*RATE),buffer.byteLength/16-startSample),stride=Math.max(1,Math.ceil(sampleCount/max)),applyFilter=(forced.filter||params.get("filter")||"display")!=="raw",data={};
    leads.forEach(lead=>{let values=Array.from({length:sampleCount},(_,offset)=>sourceLead(view,startSample+offset,lead));if(applyFilter)values=displayFilter(values);data[lead]=values.filter((_,offset)=>offset%stride===0)});
    return {start_s:round(startSample/RATE,3),duration_s:round(sampleCount/RATE,3),sample_rate_hz:RATE,display_sample_rate_hz:RATE/stride,stride,units:"µV",filter:applyFilter?"0.5–40 Hz display filter":"raw",calibration_note:"用户确认模拟数据 · 已上传源 DATA 片段 · 未建立计量学溯源",leads:data,beats:beats(UPLOADED_INDEX).filter(x=>x.time_s>=start&&x.time_s<=start+duration),annotations:annotationList(uploaded.case_id).filter(x=>x.sample_index/RATE>=start&&x.sample_index/RATE<=start+duration)};
  }
  async function waveform(index,params,forced={}){return index===UPLOADED_INDEX&&uploaded?sourceWaveform(params,forced):formulaWaveform(index,params,forced)}
  ...
  function eventData(index,params){const type=params.get("type")||"all",brady=Number(params.get("brady")||50),tachy=Number(params.get("tachy")||120),pause=Number(params.get("pause")||2.5),limit=Number(params.get("limit")||500),summary={AF:0,V:0,S:0,pause:0,tachy:0,brady:0,noise:0},items=[];beats(index).forEach(b=>{let kind,label,severity;if(b.af_onset){kind="AF";label="房颤样候选片段（纯合成）";severity="high"}else if(b.group===2){kind="S";label="室上性候选心搏";severity="medium"}else if(b.group===3){kind="V";label="室性候选心搏";severity="high"}else if(b.group===34){kind="noise";label="噪声/待确认心搏";severity="low"}else if(b.rr_ms>=pause*1000){kind="pause";label=`长 RR 间期 ${(b.rr_ms/1000).toFixed(2)}s`;severity="high"}else if(b.hr>=tachy){kind="tachy";label=`心动过速候选 ${Math.round(b.hr)} bpm`;severity="medium"}else if(b.hr<=brady){kind="brady";label=`心动过缓候选 ${Math.round(b.hr)} bpm`;severity="medium"}else return;summary[kind]++;if(type==="all"||type===kind)items.push({...b,type:kind,label,severity,review_status:"待复核"})});return {summary,total:items.length,offset:0,limit,items:items.slice(0,limit)}}
  ...
  async function route(url,options={}){
    ...
    if(path==="/api/dashboard"){const cases=Array.from({length:COUNT},(_,i)=>present(i));return response({totals:{cases:COUNT,recording_hours:round(cases.reduce((sum,item)=>sum+item.technical.duration_seconds_raw,0)/3600,2),beats:cases.reduce((n,x)=>n+x.summary.total_beats,0),pending_reports:cases.filter(x=>report(x).status!=="reviewed").length},cases,privacy:{phi_visible:false,synthetic_only:true,raw_patient_data:false}})}
    if(path==="/api/cases")return response({items:Array.from({length:COUNT},(_,i)=>present(i)),total:COUNT});
    if(path==="/api/settings")return response({app_name:"CardioInsight Holter 模拟数据在线演示",version:"0.12.0-static-demo",data_root:"1 例用户确认模拟源片段 + 9 例浏览器公式病例",case_count:COUNT,integrity_manifest:{available:true,case_count:COUNT,algorithm:"SHA-256 + DETERMINISTIC-SYNTHETIC-V3",raw_patient_data:false,user_confirmed_synthetic:true},platform:{name:"Web",release:"Cloudflare Pages / GitHub Pages",machine:"Browser",storage_root:"localStorage（仅演示修改）",config_path:"无本地配置"},clinical_use:false});
    ...
    if(action==="open"&&method==="POST"){audit("case.open",caseId,"打开公开模拟病例");return response({ok:true})}
    if(action==="trend")return response(trend(index));if(action==="hrv")return response(hrv(index));if(action==="rr-visuals")return response(rrVisuals(index));if(action==="waveform")return response(await waveform(index,url.searchParams));
  ...
  document.addEventListener("DOMContentLoaded",()=>document.querySelector("#downloadReport")?.addEventListener("click",event=>{event.preventDefault();event.stopImmediatePropagation();const text=`CardioInsight Holter 模拟数据在线演示\n${document.querySelector("#reportCaseLabel")?.textContent||"尚未选择病例"}\n\n含一例用户确认可公开的模拟源片段，不含真实病例，不可用于临床用途。\n`,link=document.createElement("a");link.href=URL.createObjectURL(new Blob([text],{type:"text/plain;charset=utf-8"}));link.download="CardioInsight_模拟数据演示说明.txt";link.click();setTimeout(()=>URL.revokeObjectURL(link.href),1000)},true));
})();
