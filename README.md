...
- 单屏诊断报告编排工作台：事件索引、最快/最慢 RR/NN 图条模式、20 类报告页选择、综合/节律/简洁模板、首页/事件/全导联预览、源报告影像、结论编辑、页面设置、打印预览、草稿/审核/驳回和中文 PDF 导出。
...
- 从侧栏进入“报告”后，页面固定为左侧编排、中间预览、右侧统计三栏，支持的桌面视口不产生页面级上下滚动。左侧“报告页”复刻参考系统的内容选择，可勾选封面、首页报告、小时统计、散点图、事件图条、房颤/房扑和 HRV 等页面；点击页面名称可立即切换中间预览。
- “最快最慢心率图条打印模式”可在 RR、NN、RR+NN 三种编排方式间切换；综合动态心电、心律失常复核和简洁医生报告三个模板会一次性设置常用页面组合，之后仍可继续自定义。
- 中间上方按参考系统提供“综合报告、事件图条、小时统计、散点图、ST 趋势、T 波趋势、房颤/房扑、HRV、全导联”模块入口；综合报告采用“双趋势 + 左侧代表图条 + 右侧同步 12 导联”的非对称编排，事件页采用“事件索引 + 代表图条 + 12 导联详情”。事件图条和全导联预览读取当前病例的真实窗口化波形，点击事件索引或图条可在原视图内更新右侧详情。页面设置支持 A4/A3、横纵向、走纸速度、增益、网格和导联标记。
- 本地开发版会把报告页面组合、当前页、预览模式和页面设置与结论一起保存到 SQLite，并进入报告版本和审计流程。在线 Demo 不改写服务器数据，编排选择仅保存在当前浏览器 `localStorage`。
- ST 趋势、T 波趋势、HRT、QTd、VCG、DC、TWA、VLP、SAP 等尚无经验证结构化数据的模块仍可作为缺项说明页加入编排，但界面会明确标为“未解析”，不会推算数值或自动生成诊断。
- “打印预览”打印当前中间页面；本地“导出 PDF”生成研究复核摘要，包含统计对照、编排清单、医生结论、环节确认记录和保留事件的采样点索引。它不是所选全部波形页的完整打印包；波形图条仍在工作站预览。静态 Demo 提供浏览器打印与演示说明下载，不冒充服务端 PDF。原报告影像继续受身份信息授权开关保护，在线 Demo 不公开源报告页面。
