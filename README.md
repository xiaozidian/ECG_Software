...
| 后端 | Python / Flask API | 无后端；浏览器 API 读取固定源数据，复核修改保存在本机浏览器 |
| 数据 | 本机 `config.json` 指向的病例目录 | 仅“徐有德”一例 10 分钟公开片段 |
| 配置 | 项目根目录本机 `config.json`，不提交 | 不读取 `config.json`、`.env` 或 `ECG_DATA_ROOT` |
| 写入 | 标注、报告、审计写入系统用户数据目录 | 不写服务器；标注、报告、心搏及房颤复核保存在当前浏览器 localStorage |
...
本软件的目标不是把六个分析页面并列堆在侧栏，而是帮助医生在同一病例上下文中，从原始证据一直走到可追溯报告：

1. **波形复核**：先确认原始波形、导联、信号质量和散点中的可疑心搏，所有后续统计都能回到这里的时间点。
2. **模板编辑**：再复核源 EBI 导入并叠加医生修订的 N / S / V / X 分组，框选形态子簇建立医生模板，或在逐搏模板库中人工改型。
3. **趋势与 HRV**：在模板和主要伪差被核对后查看全天趋势、RR 分布与 HRV 对照，避免把未复核异常 RR 直接解释为结论。
4. **ST‑T 复核**：以 16 秒正常代表搏块、局部参考、自动 J 点和 J+60 ms 相对变化生成 ST 压低/抬高及 T 波极性反转的研究筛查候选；候选只用于排列医生复核优先级，必须回看原始波形、修正或排除后才可进入报告。当前版本不生成自动 ST‑T 诊断，也不把设备原始单位换算成临床 mV。
5. **事件候选**：集中检查过速、过缓、长 RR、室性、室上性和噪声候选，并回到对应原始波形确认。
6. **报告**：最后选择报告内容、核对统计与图条、编辑医生结论、保存草稿并进入审核或导出流程。

选择病例后，顶部固定显示病例、当前任务和五个复核环节的确认状态，最后进入报告。点击导航或“仅浏览下一步”不会确认医学复核；“确认本步并继续”需要医生主动勾选确认，可附备注。本地开发版写入 SQLite 和审计，静态 Demo 仅保存到当前浏览器，不上传。

- 工作台按未确认环节提供“开始/继续复核”，已审核病例进入报告。
- 事件可标记“保留为证据 / 已排除 / 待复核”；列表分页，回看波形后可返回原列表位置。保留事件进入报告审核前检查和定位索引，不等于确诊。
- 五个复核环节未确认、结论为空或有未保存修改时，不能审核报告；浏览页面不会绕过此检查。
- 人工改型会使相关复核确认失效，已审核报告回到草稿，需重新审核；保留的事件也会重新待核。源 DATA / EBI 保持不变。
- **右键修订已驱动 RR、HRV、分类数量和逐搏事件候选重算。** 波形、散点、模板及报告重算区读取当前修订；源报告正文及医生结论不自动改写。算法仍需独立临床验证，不能直接用于诊断。

详见 [医生复核工作流](docs/医生复核工作流.md) 和 [竞品调研与本轮交互设计](docs/clinical-workflow-design.md)。

### 完整右键心搏修订（本地版 / Demo 共用）

波形复核、散点圈选图条、模板编辑连续波形和代表图条共用右键菜单，也可按 Shift+F10。支持截图中的全部可见类型和操作：改型、起搏细分、删除、批量添加 QRS、5 ms 精确移位、前后搜索候选、修改相邻心搏、指定最长 RR 报告图条、全选／反选和设置。没有展开的子菜单按公开软件流程推定，不宣称复现分规私有算法。详见 [竞品调研、完整功能矩阵及算法边界](docs/beat-editor-research.md)。

- 自动搜索读取原始导联，通过导数能量、MAD 阈值、局部峰和不应期排重生成候选；默认不勾选，医生核对波形后才写入，新增 QRS 默认为未分类。
- O（房早未下传）、Y（P 波）、T（T 波）是非心搏标记，不计入心搏／NN；旧版 O=其他保留为 OTHER。医院快捷键不是 WFDB 编码。
- AF／房扑、起搏及传导类型都是医生人工指定；没有伪装成自动疾病识别。NN 差分不跨异位搏／伪差；短记录 HRV 不能等价于完整 Holter。
- 批量、删除、补标和移位先预览重算影响。可用撤销／重做按钮，或 Ctrl/⌘+Z、Ctrl/⌘+Shift+Z；修订版本、历史和审计持久化。模板成员引用跟随心搏移动及撤销。
- 本地版保存于本机 SQLite；Demo 保存于该浏览器的独立 localStorage，存储失败回滚，不上传编辑数据。不改动正式 config，不改写源 DATA/EBI。
- 计算接口默认保留源比较兼容；界面使用 analysis=edited 读取修订结果。源报告摘要明确保留原文，医生结论不自动改写。

### 医院总览、房颤分析与波形密度图

总览新增全程／小时心率趋势、全程／小时 RR 时间散点、RR／相邻 R/R 比值直方图与对数计数显示；共用时间定位，联动连续波形、相邻 RR Lorenz 散点及圈选心搏图条。结论统计并列显示源报告总心搏估算与当前有效心搏、伪差、室早、房早和长 RR 候选。

右侧先显示“散点图复核”，再显示“结论统计”；窄屏下仍保持这一顺序。散点画布随图框实际宽高重绘，展开键盘范围选择或改变窗口大小不会留下旧尺寸空白，也不改变 RR 数据与所选坐标范围。

- 右键全程／小时 RR 图，或点标题右侧“操作”，进入房颤／房扑分析：添加、调整、切分移除、整段标记、复核状态与撤销／重做。分析页含全程 RR、时段 Lorenz、片段列表、四个 15 分钟 RR 面板及连续波形。
- 右键 Lorenz 可改型、修改相邻心搏、批量插入 QRS、前后搜索候选、排除所选 RR 间期对应的房颤／房扑，以及切换图型、框选方式、范围、点大小和辅助线。心搏写入沿用既有预览／确认流程。
- 右键心率图可插入报告图条、打印、插搏、选取边界、撤销式删除区段内心搏标记、使用三种测量尺、跳时、查看波形／事件、指定最快／最慢心率报告图条并保存定位书签，以及筛查房早候选。
- 模板编辑的彩色图是 **R 峰对齐的心搏波形密度图**，不是 RR 散点图：横轴为相对 R 峰时间，纵轴为设备幅度，颜色为全类别／模板集合的重复密度，支持框选形态。下方代表图条最多预览 24 搏，不影响全集合计数。

本地与 Demo 共用图表／交互代码；Demo 仍使用原来的单病例 10 分钟数据，不扩写为全天。短于 15 分钟的记录只展示实际覆盖的一个面板。更新公共源码后运行 `python scripts/build_static_demo.py --output docs/demo`，再按上文 Cloudflare 流程发布；不要手改 `docs/demo`。房颤修改仅保存于本机 SQLite／Demo 浏览器，不会改变 DATA、EBI 或正式 config。

自动重分析仅生成 RR 规则候选，不包含 P 波识别，不能据此自动确诊房颤／房扑或房早；只有医生确认的片段才进入“确认负荷”。详细操作映射、口径和限制见 [医院总览与房颤工作台说明](docs/overview-workbench.md)。
...
- 8 路原始通道及推导 12 导联；主界面固定为最多 3 导联复核，右键可选择显示通道，双击或按 `F` 进入全屏 12 导联；支持 1–120 秒时间窗、增益和显示滤波。
...
- 医生心搏模板编辑工作台：首次进入默认“聚类分型”，通过页内显式按钮进入“逐搏模板库”。顶部 N/S/V/X 类别条始终可见，显示类别名称、数量和快捷键；先选源类别或医生模板，再在左侧形态集合中用矩形框出形态子簇，即可派生 V1、V2 等有明确父类别的新模板。主要操作区在支持的桌面视口内固定排布，不需要页面级上下滚动。
- 心动过缓、心动过速、长 RR、室上性和室性候选事件筛选。
- ST‑T 医生复核工作台：全天定位、12 导联设备原始尺度趋势、自动 J 点与 ST60 持续性候选、最多三导联原始波形、ISO/J/ST60/ST80 手工游标、源报告线索和人工复核标注。自动结果仅用于筛查排序；病例 mV 换算、自动诊断和未经医生确认的入报保持禁用。
- 诊断报告编排：事件索引、最快/最慢 RR/NN 图条模式、自选或全部 12 导联入报、A4 首页/小时统计/波形证据、源报告影像、结论编辑、打印预览、草稿/审核/驳回和中文 PDF 导出。
- 人工标注、审计日志、报告草稿/复核状态和版本记录。
...
- 键盘 `+` / `-`：放大或缩小；`0`：复位到 10 秒；`←` / `→`：切换前后窗口；`F`：进入或退出全屏 12 导联；`A`：添加人工标注。
- 主界面最多显示 3 条波形；在波形上右键或点击“显示导联”可选择 1–3 条导联，选择结果会保存在当前浏览器。双击波形进入或退出全屏 12 导联。
- 拖动波形可平移时间窗；悬停显示精确时间、当前导联和电压坐标。人工标注使用工具栏按钮或 `A` 快捷键创建。
...
- 从病例工作流进入“模板编辑”时默认展示“聚类分型”；需要逐搏改型时，点击页面右上角“逐搏模板库”。侧栏只负责进入当前工作步骤，不再用“重复点击同一入口”承担隐藏切换。
- “聚类复核”负责从源分组的形态叠加图框选子簇并派生新模板；“模板库”是独立工作台，按模板带、RR 分布/类型筛选/散点、代表心搏矩阵、连续波形和双形态叠加对照排列，不再复用聚类页布局。
- 顶部类别条始终显示 N、S、V、X 的中文名称和心搏数；点击即可切换。未选中心搏时可按 `N` / `S` / `V` / `X` 或数字 `1`–`4` 切换源类别。选中心搏后按 `N` / `S` / `V` / `X` / `P` / `O` 则执行人工改型，避免“切换类别”和“修改类型”混淆。
- 在左侧深色“模板形态集合”中按住鼠标或触控笔拖出矩形，框出与父类别不同的形态子簇；松开后会预览选中心搏并打开“派生新模板”浮层。键盘用户可输入横向和纵向百分比得到等价矩形。
- 当前形态集合由所选类别的 32 个确定性代表心搏组成；创建的模板保存实际被框中的源 EBI 心搏索引，不会自动把源类别内其他相似心搏全部改判。
- 创建时可指定类别名称、节律家族、参考导联和说明；系统会保存父子来源（例如 `V → V1`）。本地开发版写入本机 SQLite 覆盖层，在线 Demo 只写入当前浏览器 `localStorage`。
- 连续波形仅用于时间定位和上下文复核，不再承担长时间段框选建类；桌面窄屏下局部列表或代表片段可内部滚动，但主页面不上下滚动。
- 在模板矩阵或连续波形中单击选中心搏；按 `⌘` / `Ctrl` 可多选，按 `Shift` 可连选。选中后直接敲击 `N` / `S` / `V` / `X` / `P` / `O` 写入人工类型覆盖，按 `Backspace` 恢复源 EBI 类型。每次改型和恢复都记入审计。
- 自建模板和逐搏人工覆盖都不改写源 `.DATA` / `.EBI`，不会自动把人工归类变成算法诊断，也不会触发事件统计或报告结论重算。要让改判驱动下游结果，仍需增加分析版本绑定、变更差异和可重放重算链路。

### ST‑T 复核操作

- 从侧栏进入“ST‑T 复核”。全天心率概览、时间滑块和前后窗口按钮用于定位真实原始波形，不代表 ST 趋势。
- 普通视图最多显示 3 条导联；点击导联按钮、右键波形或按 `Shift + F10` 可重新选择。双击波形、按 `F` 或点击“全屏 12 导联”进入多导联复核。
- 拖动蓝色 `ISO` 和红色 `J` 游标定位等电位基线与 QRS 终点；右侧按钮可按 5 ms 微调。`ST60` / `ST80` 会随 J 点联动，也可切换为直接查看 J 点。
- 当前相对差值只以“设备原始单位”显示，不换算 mV、不套用自动阈值。形成非“无法判读”的复核描述前，需要确认已核对信号质量、基线漂移、体位变化和 R 波幅度变化。
- 本地开发版可把描述保存为人工标注；在线 Demo 只演示测量交互，不保存意见。详细依据与后续开放条件见 [`docs/ST-T复核工作台设计与医学依据.md`](docs/ST-T复核工作台设计与医学依据.md)。

### 诊断报告编排操作

- 从侧栏进入“报告”，按事件类别筛选并勾选“入报”。未确认候选可先加入**草稿**，不会自动变成诊断；审核前仍需完成复核。最快/最慢心率支持 RR、NN、RR+NN。
- 最快／最慢心率各提供每个 RR／NN 序列最多 **200 条实际候选**，分别取最快 200 条和最慢 200 条，短记录按实显示；RR+NN 并列时最多 400 条。默认均“从快到慢”，可切换“从慢到快”；每页 50 条，可直接选页。列表显示心率、RR、时间及当前预览，点击后按入报导联和默认 7 秒／至少 5 搏回看，再选择任意候选入报。候选仍是单间期瞬时心率，不是 7 秒平均心率；不按时间段合并相邻候选，避免隐藏医生需要核查的心搏。扩展候选不自动改写统计极值、原图条 ID 或医生结论。
- 入报前可选择常用 3 导联（II / V1 / V5）、全部 12 导联或自选 1–12 个导联；“新入报图条”只影响随后加入的图条。“图条设置”/“报告图条”可独立修改已有图条的导联、时长、图注和顺序。
- **默认 7 秒，至少 5 个可用心搏**：围绕所选事件取窗，若实际逐搏索引不足 5 搏则自动延长并显示最终时长。伪差和非心搏标记不计入；整段记录本身不足 5 搏会明确提示，不能凭空补搏。该规则不改写源波形或事件本身。
- 最终输出按参考医院报告改为 **A4 纵向（210 × 297 mm）黑白纸张**：检查信息/分析统计/HRV/医生结论首页 → 小时统计表 → 所选波形。长结论自动续页；表格每页最多 25 个实际时段。1–3 导联图条每页最多 3 张全宽图，4–6 导联占两行，7–12 导联独占一页，取代此前的 3 行 2 列六图布局。
- 每张波形显示记录时间、事件标题、心率/逐搏标记、RR 和下方定位缩略条。默认 7 秒对应 25 mm/s；延长时间窗后按实际时间标注走纸尺度，不虚标 25 mm/s。当前原始电压标度未校准，逐导联采用自适应幅度并标明设备单位 `u`，不可按纸上高度读取临床 mV，也不可直接比较不同导联的显示高度。源数据伪差仍保留。
- 长图注在波形页使用编号摘要，全文进入末尾“图条说明（完整图注）”页，不截断保存内容、不覆盖时间或心率。窄屏下操作栏换行，报告页签和 A4 纸张分别在自身区域横向滚动，纸张本身不缩成难以阅读的小图。
- 统计来自当前修订逐搏数据，小时分界按实际记录时钟计算；首尾不足一小时按实计入。成对、短阵和联律是可重叠的事件计次，不与总心搏相加；长 RR 是阈值候选，房颤/房扑只计医生确认片段。源报告指标与当前重算分开列示，未提供/无法计算显示 `—`，不补造数值。
- “报告”显示完整分页预览；“打印预览”打印同一套完整纸张；本地“导出 PDF”包含这些统计和实际所选导联波形，不再只是摘要。浏览器打印请使用 A4、100% 比例、无页边距并关闭浏览器自带页眉页脚。
- 本地开发版把导联、时长、顺序和结论保存到 SQLite，保留版本/审计及失效依据检查；在线 Demo 只保存在当前浏览器 `localStorage`，使用“打印 / 保存 PDF”，不改写服务器和原始公开数据。改动后重建 `python scripts/build_static_demo.py --output docs/demo`，共享同一报告引擎与样式，保持开发版/Demo 隔离。
- 原报告影像继续受身份信息授权开关保护，在线 Demo 不公开源报告页面。所有输出保留研究用途与医生复核提示，不能据此宣称临床有效性。
...
本版本虽已提供 ST‑T 人工复核界面，但仍未实现经临床验证的房颤、ST‑T、QT/QTc 或起搏器诊断算法；也未实现多人登录与完整 RBAC、HIS/PACS/DICOM/HL7 接口、可验证数字签名、PDF/A 或医疗器械注册所需的软件生命周期与风险管理体系。详见 `docs/需求追踪矩阵.md`、`docs/临床与合规边界.md` 和 `docs/ST-T复核工作台设计与医学依据.md`。
