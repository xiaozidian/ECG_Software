...
(() => {
  const RATE=200,DEFAULT_DURATION=1800,COUNT=10,STORE_KEY="cardioinsight-pages-demo-v2";
  const DISEASE_INDEX=4,DISEASE_DURATION=600,AF_WINDOWS=[[240,252]];
  ...
  const durationFor=index=>index===DISEASE_INDEX?DISEASE_DURATION:DEFAULT_DURATION;
  const afAt=(index,time)=>index===DISEASE_INDEX&&AF_WINDOWS.some(([start,end])=>time>=start&&time<end);
  const atrialTachyAt=(index,time)=>index===DISEASE_INDEX&&time>=420&&time<423;

  function beats(index){
    if(beatCache.has(index))return beatCache.get(index);
    const list=[],duration=durationFor(index),base=index===DISEASE_INDEX?72:54+index*4;let time=.62,n=0,wasAf=false;
    while(time<duration){
      const inAf=afAt(index,time),inAtrialTachy=atrialTachyAt(index,time);
      let rr=60000/base*(1+.055*Math.sin(n*.071+index)+.025*Math.sin(n*.019));
      if(inAf)rr=760*(1+.22*Math.sin(n*2.399+index)+.08*Math.sin(n*.713));
      else if(inAtrialTachy)rr=465+18*Math.sin(n*1.7);
      else if(index===DISEASE_INDEX&&time>=72&&time<78)rr=1120+70*Math.sin(n*.9);
      else if(index!==DISEASE_INDEX&&n>0&&n%503===0)rr=2680+index*18;
      rr=Math.max(360,Math.min(3000,rr));
      const nextTime=time+rr/1000;if(nextTime>=duration)break;
      const beatAf=afAt(index,nextTime),beatAtrialTachy=atrialTachyAt(index,nextTime);
      let group=1;
      if(index===DISEASE_INDEX){
        if(beatAtrialTachy)group=2;
        else if(!beatAf&&n>0&&n%401===0)group=3;
        else if(!beatAf&&n>0&&n%143===0)group=2;
        else if(n>0&&n%389===0)group=34;
      }else if(n>0&&n%(101-index%3)===0)group=3;
      else if(n>0&&n%(73+index%4)===0)group=2;
      else if(n>0&&n%347===0)group=34;
      time=nextTime;
      list.push({sample_index:Math.round(time*RATE),time_s:round(time,3),group,label:labels[group],rr_ms:Math.round(rr),hr:round(60000/rr,1),rhythm:beatAf?"AF-like":"sinus",af_candidate:beatAf,af_onset:beatAf&&!wasAf});
      wasAf=beatAf;n++;
    }
  ...
  function baseCase(index){
    ...
    const duration=durationFor(index),disease=index===DISEASE_INDEX;
    const summary={total_beats:valid.length,avg_hr:avg,min_hr:Math.round(Math.min(...rr.map(x=>60000/x))),max_hr:Math.round(Math.max(...rr.map(x=>60000/x))),ventricular_beats:counts[3],supraventricular_beats:counts[2],longest_rr_s:round(Math.max(...rr)/1000,3),sdnn_ms:disease?138:62+index,sdann_ms:disease?128:48+index,sdnn_index_ms:disease?44:36+index,rmssd_ms:disease?24:31+index*.8,pnn50_pct:disease?4:8+index*.7,triangular_index:disease?34:18+index*.5};
    const conclusion=disease
      ?"纯合成疾病特征演示：10 分钟数学信号中包含一段短暂的房颤样候选片段，并加入少量室上性、室性候选及短阵房性心动过速样片段。不规则 RR 与 P 波弱化仅用于验证界面交互；参数来自去标识后的统计范围，波形由全新公式生成，并非任何真实病例的重建，不可用于诊断。"
      :`第 ${index+1} 例纯合成浏览器演示。所有候选与统计仅展示交互，不代表真实患者或临床结论。`;
    const item={case_id:`SYNTH-${String(index+1).padStart(3,"0")}`,metadata:{name:disease?"合成房颤样候选01":`合成${num}号`,patient_id:`DEMO-${String(index+1).padStart(4,"0")}`,sex:index%2?"女":"男",age:28+index*5,bed:"在线合成演示",clinical_diagnosis:`${diagnoses[index]}（纯合成候选）`,requesting_physician:"虚构演示",start_time:`2026-08-${num} 09:00`,start_iso:`2026-08-${num}T09:00:00`,duration_text:disease?"10 分钟":"30 分钟"},summary,conclusion,technical:{sample_rate_hz:RATE,independent_channels:8,derived_leads:12,sample_format:"deterministic mathematical formula",units:"µV（合成演示，无计量含义）",duration_seconds_raw:duration,raw_size_bytes:0,report_pages:1},integrity:{manifest_available:true,algorithm:"DETERMINISTIC-SYNTHETIC-V2",file_count:0,case_sha256:"",source_version_warning:false,raw_patient_data:false},simulation_profile:disease?{kind:"aggregate-inspired-af-like",source_waveform_copied:false,af_windows_s:copy(AF_WINDOWS),note:"疾病特征经过教学化放大；不对应真实人员或真实诊断"}:null,active:true,phi_masked:true,report_image_urls:[],generated_report_url:"#synthetic-demo"};
    caseCache.set(index,item);return item;
  }
  function annotationList(caseId){const i=indexOf(caseId),disease=i===DISEASE_INDEX,initial=[{id:-(i+1),sample_index:(disease?AF_WINDOWS[0][0]:120+i*7)*RATE,lead:"II",category:disease?"rhythm":"note",label:disease?"房颤样候选片段（纯合成）":"合成演示标记",note:disease?"数学模型生成的不规则 RR 与 P 波弱化，不对应真实病例":"由浏览器演示模拟器生成",created_by:"demo-generator",created_at:"2026-08-12 09:00:00"}];return initial.concat(saved.annotations[caseId]||[])}
  ...
  function signal(t,index,lead){
    const isAf=afAt(index,t),base=index===DISEASE_INDEX?72:54+index*4,bpm=base+3*Math.sin(t/51+index);
    const normalPhase=((t+.07*index)%(60/bpm))/(60/bpm),afTime=t-AF_WINDOWS[0][0],afPhase=((afTime*1.42+.13*Math.sin(afTime*2.7)+.055*Math.sin(afTime*5.3))%1+1)%1,phase=isAf?afPhase:normalPhase;
    const pWave=isAf?0:110*gaussian(phase,.18,.035),qrs=-120*gaussian(phase,.36,.014)+1050*gaussian(phase,.39,.012)-230*gaussian(phase,.425,.018),tWave=310*gaussian(phase,.68,.075);
    const fibrillation=isAf?28*Math.sin(2*Math.PI*t*6.1+index)+15*Math.sin(2*Math.PI*t*8.7):0;
    return Math.round((pWave+qrs+tWave+fibrillation+45*Math.sin(2*Math.PI*t/8.3+index)+12*Math.sin(2*Math.PI*t*13.1+index*.3))*(scales[lead]??1));
  }
